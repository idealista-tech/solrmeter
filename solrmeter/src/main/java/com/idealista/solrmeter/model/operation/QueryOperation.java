...
public class QueryOperation implements Operation {
  ...
  protected QueryResponse executeQuery(SolrQuery query) throws IOException, SolrServerException {
	String requestMethod = SolrMeterConfiguration.getProperty(SolrMeterConfiguration.QUERY_METHOD, "GET");
	return executor.getSolrClient().query(query, METHOD.valueOf(requestMethod));
  }
